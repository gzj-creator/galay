...
{
    if (conn == nullptr || conn->socket.fd < 0 || (data == nullptr && length > 0)) {
        return result(C_IOResultInvalid, GALAY_HTTP2_ERROR_INTERNAL);
    }
    size_t sent = 0;
    while (sent < length) {
        C_IOResult io = galay_c_tcp_socket_send(&conn->socket, data + sent, length - sent, timeout_ms);
        if (io.code != C_IOResultOk) {
...
{
    if (conn == nullptr || conn->socket.fd < 0 || (out == nullptr && length > 0)) {
        return result(C_IOResultInvalid, GALAY_HTTP2_ERROR_INTERNAL);
    }
    size_t received = 0;
    while (received < length) {
        C_IOResult io = galay_c_tcp_socket_recv(&conn->socket, out + received, length - received, timeout_ms);
        if (io.code != C_IOResultOk) {
            return map_io_result(io);
        }
        if (io.bytes == 0) {
...
void galay_http2_client_destroy(galay_http2_client_t* client)
{
    ...
    if (client->conn != nullptr) {
        const galay_status_t destroyed = galay_http2_conn_destroy(client->conn);
        (void)destroyed; /* void destroy cannot propagate an OS close failure. */
        client->conn = nullptr;
    }
    delete client;
}
...
C_IOResult galay_http2_client_connect(galay_http2_client_t* client, int64_t timeout_ms)
{
    ...
    C_IOResult created = galay_c_tcp_socket_create(&conn->socket, C_IPTypeIPV4);
    if (created.code != C_IOResultOk) {
        delete conn;
        return map_io_result(created);
    }
    C_Host host{};
    copy_host(host, client->config.host, client->config.port);
    C_IOResult connected = galay_c_tcp_socket_connect(&conn->socket, &host, timeout_ms);
    if (connected.code != C_IOResultOk) {
        C_IOResult closed = galay_c_tcp_socket_close(&conn->socket);
        delete conn;
        if (closed.code != C_IOResultOk) {
            return map_io_result(closed);
        }
...
C_IOResult galay_http2_server_listen(galay_http2_server_t* server, uint16_t* out_port)
{
    ...
    C_IOResult created = galay_c_tcp_socket_create(&server->listener, C_IPTypeIPV4);
    if (created.code != C_IOResultOk) {
        return map_io_result(created);
    }
    C_Host bind_host{};
    copy_host(bind_host, server->config.host, server->config.port);
    C_IOResult bound = galay_c_tcp_socket_bind(&server->listener, &bind_host);
    if (bound.code != C_IOResultOk) {
        C_IOResult closed = galay_c_tcp_socket_close(&server->listener);
        return closed.code == C_IOResultOk ? map_io_result(bound) : map_io_result(closed);
    }
    C_IOResult listened = galay_c_tcp_socket_listen(&server->listener, 128);
    if (listened.code != C_IOResultOk) {
        C_IOResult closed = galay_c_tcp_socket_close(&server->listener);
        return closed.code == C_IOResultOk ? map_io_result(listened) : map_io_result(closed);
    }
    C_Host local{};
    C_IOResult endpoint = galay_c_tcp_socket_local_endpoint(&server->listener, &local);
    if (endpoint.code != C_IOResultOk || local.port == 0) {
        C_IOResult closed = galay_c_tcp_socket_close(&server->listener);
        if (closed.code != C_IOResultOk) {
            return map_io_result(closed);
        }
        return endpoint.code == C_IOResultOk
            ? result(C_IOResultError, GALAY_HTTP2_ERROR_IO)
            : map_io_result(endpoint);
    }
                                     ...
                                     int64_t timeout_ms)
{
    ...
    conn->socket.fd = -1;
    C_IOResult accepted =
        galay_c_tcp_socket_accept(&server->listener, &conn->socket, nullptr, timeout_ms);
...
galay_status_t galay_http2_conn_destroy(galay_http2_conn_t* conn)
{
    ...
    if (conn->owns_socket && conn->socket.fd >= 0) {
        C_IOResult closed = galay_c_tcp_socket_close(&conn->socket);
        if (closed.code != C_IOResultOk) {
            delete conn;
