...
galay_status_t status_from_tcp_result(C_IOResult result)
{
    switch (result.code) {
        case C_IOResultOk:
            return GALAY_OK;
...
C_IOResult send_all(galay_c_tcp_socket_t* socket,
                    const uint8_t* data,
                    size_t data_len,
                    int64_t timeout_ms)
{
...
C_IOResult recv_exact(galay_c_tcp_socket_t* socket,
                      uint8_t* out,
                      size_t out_len,
                      int64_t timeout_ms)
{
...
C_IOResult read_incoming_frame(galay_c_tcp_socket_t* socket,
                               int64_t timeout_ms,
                               IncomingFrame* frame)
{
...
C_IOResult read_response_frame(galay_c_tcp_socket_t* socket,
                               int64_t timeout_ms,
...
C_IOResult send_request_frame(galay_rpc_client_t* client,
                              ...
                              int64_t timeout_ms)
{
    if (client == nullptr || !client->connected || client->socket.fd < 0 ||
...
C_IOResult send_response(galay_c_tcp_socket_t* socket,
                                              ...
                                              galay_rpc_response_buffer_t* out_response)
{
    ...
    if (out_response == nullptr || client == nullptr || !client->connected ||
        client->socket.fd < 0 || service == nullptr || method == nullptr ||
        !payload_arg_valid(payload, payload_len)) {
...
galay_status_t galay_rpc_server_listen(galay_rpc_server_t* server)
{
    ...
    if (created.code != C_IOResultOk) {
        return status_from_tcp_result(created);
    }
    C_IOResult bound = galay_c_tcp_socket_bind(&server->listener, &host);
    if (bound.code != C_IOResultOk) {
        const C_IOResult closed = galay_c_tcp_socket_close(&server->listener);
        return closed.code == C_IOResultOk ? status_from_tcp_result(bound)
                                           : status_from_tcp_result(closed);
    }
    C_IOResult listened =
        galay_c_tcp_socket_listen(&server->listener, server->backlog);
    if (listened.code != C_IOResultOk) {
        const C_IOResult closed = galay_c_tcp_socket_close(&server->listener);
        return closed.code == C_IOResultOk ? status_from_tcp_result(listened)
                                           : status_from_tcp_result(closed);
    }
...
{
    if (server == nullptr || out == nullptr || server->listener.fd < 0) {
        return GALAY_INVALID_ARGUMENT;
    }
    return status_from_tcp_result(galay_c_tcp_socket_local_endpoint(&server->listener, out));
}
...
{
    if (server == nullptr || !server->listening || server->listener.fd < 0) {
        return make_io_result(C_IOResultInvalid);
    }
    ...
    C_IOResult closed = galay_c_tcp_socket_close(&accepted);
