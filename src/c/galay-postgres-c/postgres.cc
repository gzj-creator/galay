...
bool strictBase64Decode(std::string_view encoded, std::vector<uint8_t>* decoded)
{
    ...
    decoded->resize((encoded.size() / 4U) * 3U);
    ...
    if (static_cast<size_t>(decoded_size) < padding) {
        decoded->clear();
        return false;
    }
    decoded->resize(static_cast<size_t>(decoded_size) - padding);
    const size_t encoded_size = 4U * ((decoded->size() + 2U) / 3U);
    std::string canonical(encoded_size, '\0');
    if (encoded_size > static_cast<size_t>(INT_MAX) ||
        EVP_EncodeBlock(reinterpret_cast<unsigned char*>(canonical.data()),
                        decoded->data(), static_cast<int>(decoded->size())) !=
            static_cast<int>(encoded_size) || canonical != encoded) {
        decoded->clear();
        return false;
    }
...
bool base64Encode(const uint8_t* data, size_t size, std::string* encoded)
{
    ...
    const size_t encoded_size = 4U * ((size + 2U) / 3U);
    encoded->assign(encoded_size, '\0');
    return EVP_EncodeBlock(reinterpret_cast<unsigned char*>(encoded->data()), data,
...
{
    if (digest == nullptr || key == nullptr || (data == nullptr && data_size != 0) ||
        key_size > static_cast<size_t>(INT_MAX)) return false;
...
{
    if (digest == nullptr || (data == nullptr && size != 0)) return false;
    unsigned int digest_size = 0;
    return EVP_Digest(data, size, digest->data(), &digest_size, EVP_sha256(), nullptr) == 1 &&
...
bool md5Hex(std::string_view input, std::string* output)
{
    ...
    if (EVP_Digest(input.data(), input.size(), digest.data(), &digest_size,
                   EVP_md5(), nullptr) != 1 || digest_size != digest.size()) return false;
                     ...
                     std::string* client_final)
{
    if (password.size() > static_cast<size_t>(INT_MAX) ||
        state->salt.size() > static_cast<size_t>(INT_MAX) || state->iterations > INT_MAX) return false;
    std::vector<uint8_t> salted(32);
    if (PKCS5_PBKDF2_HMAC(password.data(), static_cast<int>(password.size()),
                          state->salt.data(), static_cast<int>(state->salt.size()),
                          static_cast<int>(state->iterations), EVP_sha256(),
                          static_cast<int>(salted.size()), salted.data()) != 1) return false;
...
bool md5Password(std::string_view username,
                 ...
                 std::string* output)
{
    if (salt == nullptr || output == nullptr) return false;
    std::string first;
                                 ...
                                 int64_t timeout_ms)
{
    ...
    C_IOResult configured = galay_c_tcp_socket_set_no_delay(
        &client->socket, config->tcp_no_delay ? 1 : 0);
    if (configured.code != C_IOResultOk) {
        const C_IOResult closed = galay_c_tcp_socket_close(&client->socket);
        return closed.code == C_IOResultOk ? configured : closed;
    }
    const int64_t effective_timeout = timeout_ms < 0
        ? static_cast<int64_t>(config->connect_timeout_ms) : timeout_ms;
    C_IOResult connected = galay_c_tcp_socket_connect(&client->socket, &host, effective_timeout);
    ...
    if (sent.code != C_IOResultOk) {
        const C_IOResult closed = galay_c_tcp_socket_close(&client->socket);
        return closed.code == C_IOResultOk ? sent : closed;
    }
