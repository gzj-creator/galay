...
galay_status_t mysql_caching_sha2_password_response(const char* password,
                                                    const unsigned char* salt,
                                                    size_t salt_len,
                                                    std::vector<unsigned char>* response)
{
    ...
    std::array<unsigned char, 32> hash3{};
    unsigned int digest_len = 0;
    if (EVP_Digest(password, password_len, hash1.data(), &digest_len, EVP_sha256(), nullptr) != 1 ||
        digest_len != hash1.size() ||
        EVP_Digest(hash1.data(), hash1.size(), hash2.data(), &digest_len, EVP_sha256(), nullptr) != 1 ||
        digest_len != hash2.size()) {
    ...
    combined.insert(combined.end(), hash2.begin(), hash2.end());
    combined.insert(combined.end(), salt, salt + salt_len);
    if (EVP_Digest(combined.data(), combined.size(), hash3.data(), &digest_len,
                   EVP_sha256(), nullptr) != 1 || digest_len != hash3.size()) {
        return GALAY_IO_ERROR;
    }
    response->resize(hash1.size());
    for (size_t i = 0; i < hash1.size(); ++i) {
...
bool mysql_rsa_oaep_sha1_encrypt(std::string_view plaintext,
                                 std::string_view public_key_pem,
                                 std::vector<unsigned char>* encrypted)
{
    if (encrypted == nullptr || public_key_pem.empty() ||
        public_key_pem.size() > static_cast<size_t>(INT_MAX)) {
        return false;
    }
    encrypted->clear();
    ...
    if (context == nullptr || EVP_PKEY_encrypt_init(context) <= 0 ||
        EVP_PKEY_CTX_set_rsa_padding(context, RSA_PKCS1_OAEP_PADDING) <= 0 ||
        EVP_PKEY_CTX_set_rsa_oaep_md(context, EVP_sha1()) <= 0 ||
        EVP_PKEY_CTX_set_rsa_mgf1_md(context, EVP_sha1()) <= 0) {
        EVP_PKEY_CTX_free(context);
        EVP_PKEY_free(key);
        return false;
    }
    size_t encrypted_len = 0;
    const unsigned char* input = reinterpret_cast<const unsigned char*>(plaintext.data());
    if (EVP_PKEY_encrypt(context, nullptr, &encrypted_len, input, plaintext.size()) <= 0) {
        EVP_PKEY_CTX_free(context);
        EVP_PKEY_free(key);
        return false;
    }
    encrypted->resize(encrypted_len);
    const int encrypted_ok =
        EVP_PKEY_encrypt(context, encrypted->data(), &encrypted_len, input, plaintext.size());
    EVP_PKEY_CTX_free(context);
    EVP_PKEY_free(key);
    if (encrypted_ok <= 0) {
        encrypted->clear();
...
galay_status_t mysql_auth_response_for_plugin(std::string_view plugin,
                                              const char* password,
                                              const unsigned char* salt,
                                              size_t salt_len,
                                              std::vector<unsigned char>* response)
{
    ...
    if (plugin == "mysql_native_password") {
        *response = mysql_native_password_response(password, salt, salt_len);
        return GALAY_OK;
    }
    if (plugin == "caching_sha2_password") {
        return mysql_caching_sha2_password_response(password, salt, salt_len, response);
    }
    return GALAY_UNSUPPORTED;
}

C_IOResult read_mysql_packet(galay_c_tcp_socket_t* socket,
                                     ...
                                     galay_mysql_result_set_t** result)
{
    ...
    if (client == nullptr || query == nullptr || result == nullptr ||
        !client->connected || client->socket.fd < 0) {
        return make_io_result(C_IOResultInvalid);
    }
                                               ...
                                               galay_mysql_buffer_t** out)
{
    ...
    const galay_status_t auth_status =
                                                    ...
                                                    galay_mysql_buffer_t** out)
{
    ...
    const galay_status_t auth_status =
        mysql_auth_response_for_plugin(plugin, password, salt, salt_len, &buffer->data);
    if (auth_status != GALAY_OK) {
        delete buffer;
        return auth_status;
    }
...
void galay_mysql_client_close(galay_mysql_client_t* client)
{
    ...
    if (client->socket.fd >= 0) {
        const C_IOResult closed = galay_c_tcp_socket_close(&client->socket);
        (void)closed; /* legacy void close surface cannot return cleanup failure. */
    }
                                            ...
                                            int64_t timeout_ms)
{
    if (client == nullptr || config == nullptr || client->connected ||
        client->socket.fd >= 0 || galay_mysql_config_validate(config) != GALAY_OK) {
        return make_io_result(C_IOResultInvalid);
    }
    C_Host host{};
    ...
    C_IOResult connected = galay_c_tcp_socket_connect(&client->socket,
                                                           &host,
                                                           effective_timeout);
    ...
    if (handshake.code != C_IOResultOk) {
        const C_IOResult closed = galay_c_tcp_socket_close(&client->socket);
        client->connected = false;
        return closed.code == C_IOResultOk ? handshake : closed;
    }
...
{
    if (client == nullptr || config == nullptr || !client->connected ||
        client->socket.fd < 0 || client->handshake_packet.empty()) {
        return make_io_result(C_IOResultInvalid);
    }
    ...
    if (packet_status == GALAY_OK && view.payload_len == 2 &&
        ...
        std::vector<unsigned char> encrypted;
        if (!mysql_rsa_oaep_sha1_encrypt(password_payload, public_key_pem, &encrypted) ||
            encrypted.size() > kMysqlWireMaxPacketPayload) {
            return io_result_from_status(GALAY_IO_ERROR);
        }
                                             ...
                                             galay_mysql_pipeline_result_t** result)
{
    ...
    if (client == nullptr || pipeline == nullptr || result == nullptr ||
        pipeline->queries.empty() || !client->connected || client->socket.fd < 0) {
        return make_io_result(C_IOResultInvalid);
    }
...
C_IOResult galay_mysql_client_close_async(galay_mysql_client_t* client, int64_t timeout_ms)
{
    ...
    C_IOResult close_result = galay_c_tcp_socket_close(&client->socket);
    client->connected = false;
    client->authenticated = false;
    client->handshake_packet.clear();
    return close_result;
