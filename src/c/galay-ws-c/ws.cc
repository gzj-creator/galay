...
std::string websocket_accept_key(std::string_view key)
{
    ...
    source.append(guid);
    uint8_t digest[20]{};
    if (galay_utils_sha1(source.data(), source.size(), digest, sizeof(digest)) != GALAY_OK) {
        return {};
    }
    char encoded[28]{};
    size_t encoded_size = 0;
    if (galay_utils_base64_encode(digest, sizeof(digest), encoded, sizeof(encoded),
                                  &encoded_size) != GALAY_OK) {
        return {};
    }
    return std::string(encoded, encoded_size);
}

C_IOResult write_all(galay_c_tcp_socket_t* socket,
                     const uint8_t* data,
                     size_t data_len,
                     int64_t timeout_ms)
{
...
C_IOResult write_string(galay_c_tcp_socket_t* socket,
...
C_IOResult read_http_headers(galay_c_tcp_socket_t* socket,
                             std::string* headers,
                             size_t* header_end,
                             int64_t timeout_ms)
{
    if (socket == nullptr || socket->fd < 0 || headers == nullptr ||
        header_end == nullptr) {
        return make_io_result(C_IOResultInvalid);
    }
    ...
    *header_end = 0;
    ...
    while (headers->size() <= kWsMaxHttpHeaderBytes) {
        ...
        C_IOResult recv_result =
            galay_c_tcp_socket_recv(socket, chunk, sizeof(chunk), timeout_ms);
...
C_IOResult close_after_result(galay_c_tcp_socket_t* socket, C_IOResult primary)
{
    C_IOResult closed = galay_c_tcp_socket_close(socket);
    return closed.code == C_IOResultOk ? primary : closed;
}
...
void galay_ws_client_destroy(galay_ws_client_t* client)
{
    ...
    if (client->connection != nullptr) {
        if (client->connection->socket.fd >= 0) {
            const C_IOResult closed = galay_c_tcp_socket_close(&client->connection->socket);
...
galay_status_t galay_ws_session_adopt_tcp(galay_c_tcp_socket_t* socket,
                                          ...
                                          galay_ws_session_t** out)
{
    ...
    if (socket == nullptr || socket->fd < 0 || out == nullptr) {
        return GALAY_INVALID_ARGUMENT;
    }
...
void galay_ws_session_destroy(galay_ws_session_t* session)
{
    ...
    if (session->connection != nullptr) {
        if (session->connection->socket.fd >= 0) {
            const C_IOResult closed = galay_c_tcp_socket_close(&session->connection->socket);
            (void)closed; /* void destroy cannot propagate an OS close failure. */
        }
...
C_IOResult galay_ws_connection_send_frame(galay_ws_connection_t* connection,
                                          ...
                                          int64_t timeout_ms)
{
    if (connection == nullptr || connection->socket.fd < 0 ||
                                          ...
                                          galay_ws_received_frame_t** out_frame)
{
    ...
    if (connection == nullptr || connection->socket.fd < 0 || out_frame == nullptr) {
        return make_io_result(C_IOResultInvalid);
    }
    ...
    for (;;) {
        ...
        C_IOResult recv_result =
            galay_c_tcp_socket_recv(&connection->socket, chunk, sizeof(chunk), timeout_ms);
        if (recv_result.code != C_IOResultOk) {
            return recv_result;
        }
        if (recv_result.bytes == 0) {
...
{
    if (connection == nullptr || connection->socket.fd < 0 || timeout_ms < -1) {
        return make_io_result(C_IOResultInvalid);
    }
    C_IOResult close_result = galay_c_tcp_socket_close(&connection->socket);
