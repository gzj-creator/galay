...
galay_status_t send_all(galay_etcd_client_t* client, std::string_view data)
{
    ...
    while (sent < data.size()) {
        C_IOResult result = galay_c_tcp_socket_send(&client->socket,
                                                         data.data() + sent,
                                                         data.size() - sent,
                                                         kDefaultTimeoutMs);
...
galay_status_t read_http_response(galay_etcd_client_t* client, HttpResponse& response)
{
    ...
    for (;;) {
        C_IOResult result =
            galay_c_tcp_socket_recv(&client->socket, chunk, sizeof(chunk), kDefaultTimeoutMs);
        if (result.code != C_IOResultOk) {
            return status_from_io(result);
        }
        if (result.bytes == 0) {
                                         ...
                                         galay_etcd_error_code_t* code)
{
    ...
    C_IOResult connected =
        galay_c_tcp_socket_connect(&client->socket, &host, kDefaultTimeoutMs);
    if (connected.code != C_IOResultOk) {
        const C_IOResult closed = galay_c_tcp_socket_close(&client->socket);
        client->connected = false;
        set_code(code, error_from_status(status_from_io(connected)));
        return closed.code == C_IOResultOk ? status_from_io(connected) : GALAY_IO_ERROR;
    }
