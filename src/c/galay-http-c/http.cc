...
C_IOResult send_all(galay_http_session_t* session,
                    ...
                    int64_t timeout_ms)
{
    if (session == nullptr || session->socket.fd < 0 ||
    ...
    while (sent < data_len) {
        C_IOResult result =
            galay_c_tcp_socket_send(&session->socket, data + sent, data_len - sent, timeout_ms);
                        ...
                        Parser parser)
{
    ...
    for (;;) {
        ...
        C_IOResult read_result =
            galay_c_tcp_socket_recv(&session->socket, chunk, sizeof(chunk), timeout_ms);
...
galay_status_t destroy_socket(galay_c_tcp_socket_t& socket)
{
    if (socket.fd < 0) {
        return GALAY_OK;
    }
    const C_IOResult result = galay_c_tcp_socket_close(&socket);
    return result.code == C_IOResultOk ? GALAY_OK : GALAY_IO_ERROR;
}

C_IOResult close_session_socket(galay_http_session_t* session, int64_t timeout_ms)
{
    if (session == nullptr || session->socket.fd < 0 || session->closed || timeout_ms < -1) {
        return make_io_result(C_IOResultInvalid);
    }
    C_IOResult result = galay_c_tcp_socket_close(&session->socket);
    if (result.code == C_IOResultOk) {
        session->closed = true;
    }
    return result;
...
galay_status_t create_socket(galay_c_tcp_socket_t& socket, C_IPType type)
{
    if (socket.fd >= 0) {
        return GALAY_OK;
    }
    const C_IOResult result = galay_c_tcp_socket_create(&socket, type);
    return result.code == C_IOResultOk ? GALAY_OK :
        (result.code == C_IOResultInvalid ? GALAY_INVALID_ARGUMENT : GALAY_IO_ERROR);
}
                                     ...
                                     int64_t timeout_ms)
{
    ...
    C_IOResult result = galay_c_tcp_socket_connect(&client->session.socket, endpoint, timeout_ms);
    if (result.code != C_IOResultOk) {
...
galay_status_t galay_http_server_bind(galay_http_server_t* server, const C_Host* endpoint)
{
    ...
    const C_IOResult result = galay_c_tcp_socket_bind(&server->listener, endpoint);
    return result.code == C_IOResultOk ? GALAY_OK : GALAY_IO_ERROR;
}

galay_status_t galay_http_server_listen(galay_http_server_t* server, int backlog)
{
    if (server == nullptr || server->listener.fd < 0 || backlog <= 0) {
        return GALAY_INVALID_ARGUMENT;
    }
    const C_IOResult result = galay_c_tcp_socket_listen(&server->listener, backlog);
    if (result.code != C_IOResultOk) {
...
galay_status_t galay_http_server_local_endpoint(const galay_http_server_t* server,
                                                C_Host* endpoint)
{
    if (server == nullptr || endpoint == nullptr || server->listener.fd < 0) {
        return GALAY_INVALID_ARGUMENT;
    }
    const C_IOResult result = galay_c_tcp_socket_local_endpoint(&server->listener, endpoint);
    return result.code == C_IOResultOk ? GALAY_OK : GALAY_IO_ERROR;
}
                                    ...
                                    int64_t timeout_ms)
{
    ...
    if (server == nullptr || !server->listening ||
        server->listener.fd < 0 || out_session == nullptr) {
        return make_io_result(C_IOResultInvalid);
    }
    ...
    session->socket.fd = -1;
    C_IOResult result =
        galay_c_tcp_socket_accept(&server->listener, &session->socket, out_peer, timeout_ms);
    if (result.code != C_IOResultOk) {
        delete session;
        return result;
    }
...
{
    if (server == nullptr || server->listener.fd < 0 || !server->listening || timeout_ms < -1) {
        return make_io_result(C_IOResultInvalid);
    }
    C_IOResult result = galay_c_tcp_socket_close(&server->listener);
    if (result.code == C_IOResultOk) {
                                         ...
                                         size_t data_len, int64_t timeout_ms)
{
    ...
    C_IOResult result = galay_c_tcp_socket_recv(&session->socket, data, data_len, timeout_ms);
