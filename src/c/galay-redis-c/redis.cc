...
galay_status_t galay_redis_client_disconnect(galay_redis_client_t* client)
{
    ...
    if (client->socket.fd >= 0 && galay_c_tcp_socket_close(&client->socket).code != C_IOResultOk) {
                                            ...
                                            galay_redis_reply_t** reply)
{
    ...
    while (sent < encoded_len) {
        C_IOResult send_result = galay_c_tcp_socket_send(&client->socket,
                                                              encoded + sent,
                                                              encoded_len - sent,
                                                              timeout_ms);
    ...
    for (;;) {
        C_IOResult recv_result =
            galay_c_tcp_socket_recv(&client->socket, chunk, sizeof(chunk), timeout_ms);
