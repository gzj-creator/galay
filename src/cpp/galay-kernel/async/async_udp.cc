 ...
 * @version 1.0.0
 *
 * @details AsyncUdpSocket 生命周期（create、bind、move、destroy）的具体实现。
 ...
 * @param type IP 协议类型（IPV4 或 IPV6）
 * @return 成功返回 AsyncUdpSocket；失败返回 IOError(kOpenFailed, errno)
 */
 ...
 */
std::expected<void, IOError> AsyncUdpSocket::bind(const Host& host)
{
    if (!host.valid()) {
