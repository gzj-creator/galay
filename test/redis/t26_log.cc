...
int main()
{
    galay::redis::log::set(nullptr);

    int disabled_argument_count = 0;
    REDIS_LOG_ERROR("[disabled]", "value={}", buildLogValue(disabled_argument_count));
    if (disabled_argument_count != 0 || REDIS_LOG_ENABLED(galay::kernel::LogLevel::kError)) {
        return EXIT_FAILURE;
    }
    ...
    galay::redis::log::set(std::move(logger));

    int filtered_argument_count = 0;
    REDIS_LOG_DEBUG("[filtered]", "value={}", buildLogValue(filtered_argument_count));
    if (filtered_argument_count != 0 || raw_logger->size() != 0) {
        return EXIT_FAILURE;
    }

    REDIS_LOG_WARN("[enabled]", "value={}", buildLogValue(filtered_argument_count));
    if (filtered_argument_count != 1 ||
        raw_logger->size() != 1 ||
        raw_logger->tagAt(0) != "[redis] [enabled]" ||
