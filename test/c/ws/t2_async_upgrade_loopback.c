...
static int run_loopback(void)
{
    ...
    REQUIRE_TRUE(galay_c_coro_spawn(&runtime, ws_server_entry, &state, NULL, &server).code ==
                     C_IOResultOk,
                 4);
    REQUIRE_TRUE(galay_c_coro_spawn(&runtime, ws_client_entry, &state, NULL, &client).code ==
                     C_IOResultOk,
                 5);
