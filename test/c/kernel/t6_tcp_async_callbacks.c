...
int main(void)
{
    ...
    if (send(client_fd, request, sizeof(request) - 1, 0) != (ssize_t)(sizeof(request) - 1)) {
        exit_code = 21;
        goto cleanup;
    }
    ...
    int read_bytes = wait_and_read(client_fd, client_buffer, sizeof(client_buffer));
    if (read_bytes != (int)(sizeof(response) - 1) ||
