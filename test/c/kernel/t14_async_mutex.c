...
static void lock_entry(void* ctx)
{
    ...
    state->result = galay_c_async_mutex_lock(state->mutex, -1);
...
{
    LockState* state = (LockState*)ctx;
    state->result = galay_c_async_mutex_lock(state->mutex, 20);
    atomic_store(&state->phase, 1);
}
...
int main(void)
{
    ...
    if (galay_c_async_mutex_get_error(C_AsyncMutexSuccess) == 0) {
        return 1;
    }
    if (expect_status(galay_c_async_mutex_create(0), C_AsyncMutexParameterInvalid)) {
        return 2;
    }
    if (expect_status(galay_c_async_mutex_destroy(0), C_AsyncMutexParameterInvalid)) {
        return 3;
    }
    if (galay_c_async_mutex_is_locked(0)) {
        return 4;
    }
    if (expect_status(galay_c_async_mutex_destroy(&mutex), C_AsyncMutexSuccess)) {
        return 5;
    }
    if (expect_status(galay_c_async_mutex_create(&mutex), C_AsyncMutexSuccess)) {
        return 6;
    }
    if (mutex.mutex == 0 || galay_c_async_mutex_is_locked(&mutex)) {
        return 7;
    }
    if (expect_io_code(galay_c_async_mutex_lock(0, -1), C_IOResultInvalid) ||
        expect_io_code(galay_c_async_mutex_lock(&mutex, -1), C_IOResultInvalid)) {
        return 8;
    }
    ...
    if (expect_io_code(galay_c_coro_spawn(&runtime, lock_entry, &first, 0, &first_task),
            C_IOResultOk) ||
    ...
    if (expect_status(galay_c_async_mutex_unlock(&mutex), C_AsyncMutexSuccess) ||
        expect_io_code(galay_c_coro_join(&first_task, 2000), C_IOResultOk) ||
        galay_c_async_mutex_is_locked(&mutex)) {
        exit_code = 12;
        goto cleanup;
    }
    if (expect_io_code(galay_c_coro_destroy(&first_task), C_IOResultOk)) {
        exit_code = 19;
        goto cleanup;
    }
    ...
    if (expect_io_code(galay_c_coro_spawn(&runtime, lock_entry, &first, 0, &first_task),
            C_IOResultOk) ||
        wait_phase(&first.phase, 1) != 0 ||
        expect_io_code(galay_c_coro_spawn(&runtime, lock_entry, &second, 0, &second_task),
            C_IOResultOk)) {
        exit_code = 13;
        goto cleanup;
    }
    ...
    if (expect_status(galay_c_async_mutex_unlock(&mutex), C_AsyncMutexSuccess) ||
        expect_io_code(galay_c_coro_join(&first_task, 2000), C_IOResultOk) ||
        expect_io_code(galay_c_coro_join(&second_task, 2000), C_IOResultOk) ||
        galay_c_async_mutex_is_locked(&mutex)) {
        exit_code = 16;
        goto cleanup;
    }
    if (expect_io_code(galay_c_coro_destroy(&first_task), C_IOResultOk) ||
        expect_io_code(galay_c_coro_destroy(&second_task), C_IOResultOk)) {
        exit_code = 20;
        goto cleanup;
    }
    ...
    if (expect_io_code(galay_c_coro_spawn(&runtime, lock_entry, &first, 0, &first_task),
            C_IOResultOk) ||
        wait_phase(&first.phase, 1) != 0 ||
        expect_io_code(galay_c_coro_spawn(&runtime, lock_timeout_entry, &second, 0, &second_task),
            C_IOResultOk) ||
        wait_phase(&second.phase, 1) != 0 ||
...
cleanup:
    ...
    if (mutex.mutex != 0) {
        if (galay_c_async_mutex_destroy(&mutex) != C_AsyncMutexSuccess &&
            exit_code == 0) {
            exit_code = 24;
        }
    }
