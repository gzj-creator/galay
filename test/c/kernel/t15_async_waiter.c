...
static int create_started_runtime(galay_c_runtime_t* runtime)
{
...
static void wait_entry(void* ctx)
{
    ...
    state->result = galay_c_async_waiter_wait(state->waiter, state->timeout_ms);
...
static int test_notify_before_wait(void)
{
    ...
    if (expect_status(galay_c_async_waiter_create(&waiter), C_AsyncWaiterSuccess)) {
        exit_code = 2;
        goto cleanup;
    }
    if (expect_status(galay_c_async_waiter_notify(&waiter), C_AsyncWaiterSuccess)) {
        exit_code = 3;
        goto cleanup;
    }
    if (!galay_c_async_waiter_is_ready(&waiter) ||
        galay_c_async_waiter_is_waiting(&waiter)) {
        exit_code = 4;
        goto cleanup;
    }
    if (expect_io_code(galay_c_coro_spawn(&runtime, wait_entry, &state, 0, &task),
            C_IOResultOk) ||
        expect_io_code(galay_c_coro_join(&task, 2000), C_IOResultOk) ||
...
cleanup:
    ...
    if (waiter.waiter != 0) {
        if (galay_c_async_waiter_destroy(&waiter) != C_AsyncWaiterSuccess &&
            exit_code == 0) {
            exit_code = 7;
        }
    }
...
static int test_wait_before_notify(void)
{
    ...
    for (int i = 0; i < 200; ++i) {
        if (galay_c_async_waiter_is_waiting(&waiter)) {
            break;
        }
        struct timespec pause = {0, 1000000};
        if (nanosleep(&pause, 0) != 0 && errno != EINTR) {
...
static int test_duplicate_notify(void)
{
    ...
    if (expect_status(galay_c_async_waiter_notify(&waiter), C_AsyncWaiterOperationInvalid)) {
        exit_code = 22;
        goto cleanup;
    }
...
static int test_spawn_notify_destroy_race(void)
{
    ...
    for (int i = 0; i < ASYNC_WAITER_SIGNAL_RACE_ITERATIONS; ++i) {
        ...
        if (expect_status(galay_c_async_waiter_create(&waiter), C_AsyncWaiterSuccess) ||
            expect_io_code(galay_c_coro_spawn(&runtime, wait_entry, &state, 0, &task), C_IOResultOk) ||
            expect_status(galay_c_async_waiter_notify(&waiter), C_AsyncWaiterSuccess) ||
            expect_io_code(galay_c_coro_join(&task, 3000), C_IOResultOk) ||
            state.result.code != C_IOResultOk) {
            exit_code = 41;
        }
        if (task.task != 0) {
...
int main(void)
{
    galay_c_async_waiter_t invalid = {0};
    if (galay_c_async_waiter_get_error(C_AsyncWaiterSuccess) == 0 ||
        expect_status(galay_c_async_waiter_create(0), C_AsyncWaiterParameterInvalid) ||
        expect_status(galay_c_async_waiter_destroy(0), C_AsyncWaiterParameterInvalid) ||
        galay_c_async_waiter_is_waiting(0) ||
        galay_c_async_waiter_is_ready(0) ||
        expect_status(galay_c_async_waiter_notify(0), C_AsyncWaiterParameterInvalid) ||
        expect_io_code(galay_c_async_waiter_wait(0, -1), C_IOResultInvalid) ||
        expect_io_code(galay_c_async_waiter_wait(&invalid, -1), C_IOResultInvalid)) {
        return 1;
    }
