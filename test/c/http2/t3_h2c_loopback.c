...
int main(void)
{
    ...
    REQUIRE_TRUE(galay_c_coro_spawn(&runtime, server_entry, &state, 0, &server_task).code ==
                     C_IOResultOk,
                 5);
    REQUIRE_TRUE(galay_c_coro_spawn(&runtime, client_entry, &state, 0, &client_task).code ==
                     C_IOResultOk,
                 6);
