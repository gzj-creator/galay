...
static C_IOResult send_json(galay_c_tcp_socket_t* socket, const char* body)
{
    char response[512];
...
static int run_lease_pipeline_loopback(void)
{
    ...
    REQUIRE_TRUE(galay_c_coro_spawn(&runtime, lease_pipeline_server_entry, &state, NULL, &server).code ==
                     C_IOResultOk,
                 4);
    REQUIRE_TRUE(galay_c_coro_spawn(&runtime, lease_pipeline_client_entry, &state, NULL, &client).code ==
                     C_IOResultOk,
                 5);
    REQUIRE_TRUE(galay_c_coro_join(&client, 3000).code == C_IOResultOk, 6);
    REQUIRE_TRUE(galay_c_coro_join(&server, 3000).code == C_IOResultOk, 7);
