...
static int run_unary_loopback(void)
{
    ...
    REQUIRE_TRUE(galay_c_coro_spawn(&runtime, rpc_server_entry, &state, NULL, &server_task).code ==
                     C_IOResultOk,
                 9);
    REQUIRE_TRUE(galay_c_coro_spawn(&runtime, rpc_client_entry, &state, NULL, &client_task).code ==
                     C_IOResultOk,
                 10);
