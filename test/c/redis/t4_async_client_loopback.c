...
static void redis_server_entry(void* arg)
{
    ...
    state->server_recv_result =
        galay_c_tcp_socket_recv(&state->accepted, state->request, sizeof(expected) - 1, 1000);
    ...
    state->server_send_result =
        galay_c_tcp_socket_send(&state->accepted, response, sizeof(response) - 1, 1000);
...
static int run_loopback(void)
{
    ...
    REQUIRE_TRUE(galay_c_coro_spawn(&runtime, redis_server_entry, &state, NULL, &server).code ==
                     C_IOResultOk,
                 4);
    REQUIRE_TRUE(galay_c_coro_spawn(&runtime, redis_client_entry, &state, NULL, &client).code ==
                     C_IOResultOk,
                 5);
