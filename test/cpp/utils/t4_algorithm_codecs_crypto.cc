...
void test_murmur_hash3() {
    ...
    // Hashing arbitrary byte buffers must not require integer alignment.
    alignas(uint64_t) unsigned char alignedData[32] = {
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
        0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
        0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
        0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F
    };
    alignas(uint64_t) unsigned char unalignedStorage[sizeof(alignedData) + 1] = {};
    std::memcpy(unalignedStorage + 1, alignedData, sizeof(alignedData));
    volatile size_t unalignedOffset = 1;
    const unsigned char* unalignedData = unalignedStorage + unalignedOffset;
    if (MurmurHash3Util::Hash128Raw(unalignedData, sizeof(alignedData)) !=
        MurmurHash3Util::Hash128Raw(alignedData, sizeof(alignedData)) ||
        MurmurHash3Util::Hash32(unalignedData, sizeof(alignedData)) !=
        MurmurHash3Util::Hash32(alignedData, sizeof(alignedData))) {
        std::cerr << "MurmurHash3 result changed for an unaligned input buffer" << std::endl;
        std::abort();
    }
