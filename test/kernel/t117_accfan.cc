/**
 * @file t117_accfan.cc
 * @brief 用途：锁定同一 IO scheduler 内 accept 与并发 connect 共存时，所有 connect 都必须恢复完成。
 * 关键覆盖点：`AcceptAwaitable` 与 `ConnectAwaitable` 共享同一后端事件循环时的 fanout 调度。
...
void waitForClients(TestState& state)
{
    ...
    while (state.client_done.load(std::memory_order_relaxed) < kConnections) {
        if (state.failed.load(std::memory_order_acquire)) {
            std::lock_guard<std::mutex> lock(state.failure_mu);
            throw std::runtime_error(state.failure.empty() ? "same-scheduler connect failed" : state.failure);
        }
        if (std::chrono::steady_clock::now() >= deadline) {
            throw std::runtime_error(
                "same-scheduler connect timed out [client_done=" +
