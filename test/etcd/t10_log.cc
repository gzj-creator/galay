/**
 * @file t10_log.cc
 * @brief 验证 galay-etcd 库级日志入口与空 logger 过滤路径。
 */
...
int main()
{
    galay::etcd::log::set(nullptr);

    int disabled_argument_count = 0;
    ETCD_LOG_ERROR("[disabled]", "value={}", buildLogValue(disabled_argument_count));
    if (disabled_argument_count != 0 || ETCD_LOG_ENABLED(galay::kernel::LogLevel::kError)) {
        return EXIT_FAILURE;
    }
    ...
    galay::etcd::log::set(std::move(logger));

    int filtered_argument_count = 0;
    ETCD_LOG_DEBUG("[filtered]", "value={}", buildLogValue(filtered_argument_count));
    if (filtered_argument_count != 0 || raw_logger->size() != 0) {
        return EXIT_FAILURE;
    }

    ETCD_LOG_WARN("[enabled]", "value={}", buildLogValue(filtered_argument_count));
    if (filtered_argument_count != 1 ||
        raw_logger->size() != 1 ||
        raw_logger->tagAt(0) != "[etcd] [enabled]" ||
