...
| 目录 | 职责 |
|---|---|
| `mongo/base` | 基础类型：`MongoConfig`、`MongoError`、`MongoValue`、库级日志入口 |
| `mongo/protoc` | BSON 编解码、Mongo `OP_MSG` 协议封装、同步连接收发 |
| `mongo/sync` | `MongoClient` 同步会话 API 与同步 SCRAM 认证 |
| `mongo/async` | `AsyncMongoClient` 协程 API、awaitable 状态机、异步 SCRAM 认证 |
| `mongo/module` | C++23 模块接口 `galay.mongo` |
...
- 客户端 nonce 通过 `galay::utils::SaltGenerator::generateSecureBytes` 生成。
- 认证计算链：
  - `PBKDF2-HMAC-SHA256`
...
| 日志入口 | `galay::mongo::log::set/get` | `galay::mongo::log::set/get` |
