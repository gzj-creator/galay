...
在当前统一仓库的安装导出中，等价稳定入口是 `galay::rpc` 与 `galay::kernel`；源码树内仍保留 `galay-rpc::galay-rpc`、`galay-kernel::galay-kernel` 兼容 alias。
...
当前 AppleClang 路径会明确跳过 `galay-rpc-modules`，继续降级到 include 路径。检查工具链版本：
