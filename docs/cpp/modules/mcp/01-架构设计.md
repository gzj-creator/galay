...
│  McpClient/Server  │  McpClient/Server         │
├─────────────────────────────────────────────────────────┤
│           协议层 (Protocol Layer)                        │
...
class McpError {
public:
    McpErrorCode code() const;           // 错误码（枚举类型）
...
```cpp
// 服务器协程处理
kernel::Task<void> McpHttpServer::handleRequest(http::HttpConn& conn) {
    auto body = co_await conn.readBody();
    auto response = processRequest(body);
    if (auto __await_result = co_await conn.writeResponse(response); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
}
```
...
// 异步发送响应
if (auto __await_result = co_await conn.writeResponse(result); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
```cpp
server.addResource("file:///data.txt", "Data file", "Data file resource", "text/plain",
