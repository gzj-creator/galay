...
│                   galay-kernel 运行时                     │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌───────────┐  │
│  │ Runtime  │ │AsyncTcpSocket │ │Task<void> │ │RingBuffer │  │
│  │          │ │(platform │ │          │ │           │  │
│  │          │ │ backend) │ │          │ │           │  │
│  └──────────┘ └──────────┘ └──────────┘ └───────────┘  │
└─────────────────────────────────────────────────────────┘
...
- **Type**: 消息类型（见下表）
- **Flags**: 调用模式与 end-of-stream 标志
- **Reserved**: 扩展位；`0x01` 表示 REQUEST body 携带 metadata 扩展，其它未知 bit 会被拒绝为 `INVALID_REQUEST`
...
galay-rpc (header-only)
    │
    ├── galay-kernel    (协程运行时、AsyncTcpSocket、RingBuffer、AsyncMutex)
    │       │
