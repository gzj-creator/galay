...
**A:** `galay-rpc` 通过 `find_package(galay-kernel 5.2.0 CONFIG REQUIRED)` 与 `find_package(galay-utils 3.2.0 CONFIG REQUIRED)` 查找依赖。若你按 README 的并排 clone 方式组织仓库，请先为 `galay-kernel` 与 `galay-utils` 生成 package config：
...
cmake -S . -B build -DCMAKE_PREFIX_PATH=../.galay-prefix/latest
```

本仓库现在只走标准 `find_package(...)`；如果依赖不在默认安装前缀，可显式指定：
...
在当前统一仓库的安装导出中，稳定入口是 `galay::rpc` 与 `galay::kernel`。如果启用了 `GALAY_RPC_ENABLE_ETCD=ON`，etcd 服务发现适配也通过同一个 `galay::rpc` 目标传播。
...
- 支持 CMake C++ module dependency scanning 的编译器（Clang 16+、GCC 14+、MSVC 14.34+）

当前 AppleClang 路径会明确跳过 RPC module file set，继续降级到 include 路径。检查工具链版本：
...
- 使用 `RpcClient::heartbeat()`/managed client 健康检查尽早发现断链
- 客户端检测连接状态并按 `RpcReconnectPolicy` 自动重连
- 调整防火墙超时设置
...
```cpp
Task<void> connectWithRetry(RpcClient& client, const std::string& host, uint16_t port) {
    int retries = 0;
...
public:
    ...
    Task<void> myMethod(RpcContext& ctx) {
        // ...
    }
};
```
...
// 服务端
Task<void> method(RpcContext& ctx) {
...
3. 服务端是否正确处理流初始化：
   ...
   public:
       ...
       Task<void> streamMethod(RpcStream& stream) {
...
```cpp
Task<void> streamWriter(StreamWriter& writer) {
    for (int i = 0; i < 100; ++i) {
...
Task<void> streamReader(StreamReader& reader) {
    while (true) {
        StreamMessage msg;
        while (true) {
...
Task<void> bidirectionalStream(RpcClient& client) {
...
**A:** 以当前仓库中的 `cmake/option.cmake`、示例和烟测为准，单一可信口径如下：

- **已明确建模并在文档中承诺的后端**
  - Linux：默认 `epoll`；显式设置 `-DDISABLE_IOURING=OFF` 且检测到 `liburing` 时使用 `io_uring`
  - macOS：`kqueue`
- **实验性分支**
  - Windows：CMake 会选择 `IOCP` 分支并定义 `USE_IOCP`，但本仓库没有原生 Windows 的 CI / 示例烟测覆盖，因此不按“已验证平台”承诺
- **当前不承诺支持**
  - FreeBSD 和其他 Unix-like 系统在本仓库内没有独立后端分支，也没有仓库级验证，请按未声明支持处理
