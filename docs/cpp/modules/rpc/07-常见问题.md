...
**A:** 当前统一仓库构建时由顶层 CMake 提供内部依赖 target。安装后只导出 `galay` 一个 CMake package，外部项目使用 `find_package(galay CONFIG REQUIRED)` 后链接 `galay::rpc`。

```bash
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release -DGALAY_BUILD_RPC=ON
...
```bash
export CMAKE_PREFIX_PATH="/path/to/galay/install"
cmake -S . -B build
```
