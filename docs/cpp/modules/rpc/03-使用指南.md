...
| GALAY_ENABLE_CPP23_MODULES | OFF | 工具链满足时生成 C++23 `import` 模块 target |
| GALAY_DISABLE_IOURING | ON | Linux 下禁用 `io_uring`，固定使用 `epoll` |

> `GALAY_ENABLE_CPP23_MODULES=ON` 需要 CMake `>= 3.28`、`Ninja`/`Visual Studio` 生成器，以及支持 CMake module dependency scanning 的编译器。当前 AppleClang 路径会明确跳过 `galay-rpc-modules`，继续使用 include 路径。
> `galay-rpc::galay-rpc` 是默认消费 target；当前仓库没有共享库 / 静态库切换选项。
> `GALAY_DISABLE_IOURING=OFF` 时，Linux 会尝试探测 `liburing` 并切换到 `io_uring`；未找到时自动回退到 `epoll`。
> `galay-rpc::galay-rpc-modules` 仅在模块 target 真实生成时导出。
