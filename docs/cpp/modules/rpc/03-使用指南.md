...
| GALAY_BUILD_BENCHMARKS | ON | 构建压测 |
| GALAY_BUILD_EXAMPLES | ON | 构建示例 |
| GALAY_ENABLE_CPP23_MODULES | OFF | 工具链满足时为 `galay::rpc` 注册 C++23 module file set |
| GALAY_RPC_ENABLE_ETCD | OFF | 将 RPC etcd 服务发现适配编译进 `galay::rpc`，要求 `GALAY_BUILD_ETCD=ON` |
| GALAY_DISABLE_IOURING | ON | Linux 下禁用 `io_uring`，固定使用 `epoll` |

> `GALAY_ENABLE_CPP23_MODULES=ON` 需要 CMake `>= 3.28`、`Ninja`/`Visual Studio` 生成器，以及支持 CMake module dependency scanning 的编译器。当前 AppleClang 路径会明确跳过 RPC module file set，继续使用 include 路径。
> `galay::rpc` 是默认消费 target；当前仓库没有共享库 / 静态库切换选项。
> `GALAY_DISABLE_IOURING=OFF` 时，Linux 会尝试探测 `liburing` 并切换到 `io_uring`；未找到时自动回退到 `epoll`。
> RPC 的 C++23 module file set 与 etcd 扩展都挂在 `galay::rpc` 上，不再导出独立的 module 或 rpc-etcd target。
...
target_link_libraries(your_target PRIVATE galay::rpc)
```
...
public:
    ...
    Task<void> method1(RpcContext& ctx) {
        // 从请求中读取数据
    ...
    Task<void> method2(RpcContext& ctx) {
        // 设置错误
...
RpcServer server(config);
if (auto result = server.registerService(echoService); !result) {
    return 1;
}
if (auto result = server.registerService(calcService); !result) {
    return 1;
}
if (auto result = server.registerService(userService); !result) {
    return 1;
}
...
每个服务通过 `serviceName` 区分，客户端调用时需指定服务名和方法名。服务器只借用服务实例，服务必须存活到服务器停止之后；单个服务器最多注册 64 个服务。
...
config.parallel_scheduler_count = 2;   // 计算调度器数量
...
```cpp
Task<void> streamExample(RpcClient& client) {
    // 先建立连接
    if (auto __await_result = co_await client.connect("127.0.0.1", 9000); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
...
`AsyncLocalServiceRegistry` 的异步方法返回 `Task<void>`，同一实例的同一方法不能被多个协程并发 `co_await`。多协程场景下：
