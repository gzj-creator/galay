...
**A:** 不要在项目里硬编码 `/usr/local`；让 `find_package(...)` 通过 `CMAKE_PREFIX_PATH` 或 `<Package>_DIR` 发现依赖：
...
**A:** 优先链接导出的 target，而不是手动把传递依赖逐个写死：

```cmake
find_package(galay-http CONFIG REQUIRED)
target_link_libraries(your_target PRIVATE galay-http::galay-http)

# 仓库内子目录 target 直接链接本项目目标即可
...
reader_setting.setMaxBodySize(100 * 1024 * 1024);  // 100MB
...
```cpp
std::string forwarded_for = req.header().headerPairs().getValue("X-Forwarded-For");
...
```cpp
auto result = co_await session.get("/api/data").timeout(std::chrono::seconds(10));

if (!result) {
    std::cerr << "请求失败: " << result.error().message() << '\n';
} else if (!result.value().has_value()) {
    std::cerr << "请求仍在进行中，可继续等待或重试 co_await\n";
}
```
...
```cpp
while (true) {
    auto result = co_await session.get("/redirect");
    if (!result) {
        std::cerr << "请求失败: " << result.error().message() << '\n';
        break;
    }
    ...
    if (response.header().code() == HttpStatusCode::Found_302) {
        std::string location = response.header().headerPairs().getValue("Location");
        if (!location.empty()) {
            auto redirect_result = co_await session.get(location);
            if (!redirect_result) {
                std::cerr << "重定向请求失败: "
                          << redirect_result.error().message() << '\n';
            }
        }
    }
    break;
...
解决方案：当前公开 `WsReaderSetting` 没有“自动 Ping/Pong 开关”；如需保活，请保持正常读循环，并由应用层主动发送 Ping：
...
Task<void> heartbeat(WsWriter& writer) {
    while (true) {
...
for (int i = 0; i < 10; ++i) {
    auto stream = client.get("/batch/" + std::to_string(i));
