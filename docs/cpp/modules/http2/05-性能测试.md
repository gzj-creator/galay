...
按全模块计划申请并执行了 `brew install h2o`。Homebrew 可解析 h2o 2.2.6 及 OpenSSL 依赖，
但 bottle 从 `ghcr.io` 下载时返回 `curl: (35) Recv failure: Connection reset by peer`，安装失败。
当前 `h2o` 仍不在 `PATH`，但这不代表 nghttp2 缺失：本机
`nghttp`、`nghttpd`、`nghttpx`、`h2load` 与 pkg-config `libnghttp2` 均为 1.69.0。

P0 HTTP/2 POST echo 已由上文的干净 `libnghttp2` h2c echo server 完成同构排名。
按“一模块一个业界标杆”的收敛口径，h2o/nginx 不再是完成条件；当前主排名缺口只剩
`nghttp2-echo` 源码/build 接线的可复现性恢复。完整 max-streams matrix 与 TLS/h2 属于可选扩展。

可复现性复核：当前仓库能找到正式 CSV、stdout 和图表，但未找到该手写 `nghttp2-echo`
server 的源码或 CMake target。因此已有数据继续有效归档，但 fresh rerun 仍需先把 competitor harness
源码恢复到 `benchmark/` 并接入构建/脚本。
