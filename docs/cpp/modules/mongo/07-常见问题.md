...
**A:** 确保 CMake 能找到统一安装的 `galay` 包，使用 `CMAKE_PREFIX_PATH` 指定：
...
如果启用 TLS，请在构建 galay 时打开 `GALAY_BUILD_SSL=ON` 并通过 `galay-ssl` 获得 SSL 能力，不要在 Mongo consumer 中直接链接底层 TLS 库。
...
1. 定期 ping 保活：
   ```cpp
   if (auto ping = co_await client.ping("test"); !ping) {
       std::cerr << "ping 失败\n";
   }
   ```
...
2. 检测连接状态并重连：
   ```cpp
   if (client.isClosed()) {
       if (auto conn = co_await client.connect(cfg); !conn) {
           std::cerr << "重连失败\n";
       }
   }
   ```
...
if (result && result->ok()) {
    // 检查 modifiedCount
    const auto* modified = result->document().find("nModified");
    if (modified && modified->isInt32()) {
        std::cout << "修改了 " << modified->toInt32() << " 个文档\n";
    }
}
```
...
// 正确
scheduler->spawn([](IOScheduler* sched) -> Task<void> {
...
**A:** 当前只设置连接超时；发送/接收超时由异步客户端的等待体超时承担：
...
if (oid && oid->isBinary()) {
    const auto& binary = oid->toBinary();
...
if (date && date->isInt64()) {
    int64_t timestamp_ms = date->toInt64();
    // 转换为 time_t
