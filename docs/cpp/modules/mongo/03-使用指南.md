...
```cpp
Task<void> run(galay::kernel::IOScheduler* scheduler, galay::mongo::MongoConfig cfg) {
...
```cpp
Task<void> runPipeline(galay::kernel::IOScheduler* scheduler, galay::mongo::MongoConfig cfg) {
    galay::mongo::AsyncMongoClient client(scheduler);
...
`AsyncMongoClient` 的连接日志由 `galay::mongo::log::set()` 注入的库级 logger 控制。未设置 logger 时不会格式化日志消息。
