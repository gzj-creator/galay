...
filter.append("age", std::move(age_cond));
...
group_spec.append("total", std::move(sum_expr));

MongoDocument avg_expr;
avg_expr.append("$avg", "$price");
group_spec.append("avgPrice", std::move(avg_expr));
...
// 基本类型
doc.append("null_field", MongoValue(nullptr));
...
```cpp
Task<void> connectWithRetry(IOScheduler* scheduler,
...
```cpp
Task<void> executeWithRetry(AsyncMongoClient& client,
...
galay::mongo::log::set(std::make_unique<MyLogger>());
```

`MyLogger` 继承 `galay::kernel::BaseLogger`。未设置 logger 或日志级别低于 `minLevel()` 时，不会执行日志消息格式化。
...
nlohmann::json documentToJson(const MongoDocument& doc) {
    ...
    for (const auto& [key, value] : doc.fields()) {
        if (value.isNull()) {
            ...
            result[key] = value.toInt32();
        } else if (value.isInt64()) {
            result[key] = value.toInt64();
        } else if (value.isDouble()) {
...
Task<void> handleRequest(HttpRequest& req, HttpResponse& resp,
                        AsyncMongoClient& client) {
    MongoDocument filter;
    ...
    auto result = co_await client.command("test", std::move(find_cmd));

    if (result && result->ok()) {
        nlohmann::json json = documentToJson(result->document());
