...
```bash
brew install cmake mongodb-community
```
...
```bash
sudo apt-get update
sudo apt-get install -y cmake g++ mongodb
```
...
模块接口文件统一使用 `.cppm`（当前为 `src/cpp/galay-mongo/module/galay_mongo.cppm`）。
...
Task<void> app(IOScheduler* scheduler, State* state) {
...
确保 CMake 能找到统一安装的 `galay` 包。可通过 `CMAKE_PREFIX_PATH` 指定：

```bash
cmake -S . -B build -DCMAKE_PREFIX_PATH="/path/to/galay/install"
```
...
```cmake
target_link_libraries(your_target PRIVATE galay::mongo galay::kernel)
```
...
- 查看 [高级主题](06-高级主题.md) 了解性能优化和最佳实践
