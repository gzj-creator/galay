...
覆盖当前批次要实现的行为：
- `H2DispatchResult` 能表达 connection error / stream error / actions。
...
以下行为放到对应后续 Task 再写 RED 测试，避免当前批次验证长期失败：
- GOAWAY 后新 stream 被拒绝，已存在 stream 可继续（Task 5）。
- 空 body 且 `end_stream=true` 产生零长度 `DATA END_STREAM`（Task 7）。
- 多个 stream 同时 pending 时，低权重流不会永久饥饿（Task 8）。
...
## [x] Task 3: 补 frame_disp 帧 stream id 约束
...
## [x] Task 4: 引入最小 stream lifecycle
...
## [x] Task 8: Deficit Round Robin 调度
...
## [x] Task 9: 区分控制帧、HEADERS 与 DATA 队列
...
**Files:**
- Create: `test/http2/t85_h2pressure.cc`
- Create: `benchmark/http2/b14_h2_kernel_pressure.cc`
...
- 单 stream 大 body，验证无 `erase(0, chunk)` 级别退化和数据丢失。
...
Expected: PASS，并输出 scheduler/flow/dispatch 压力指标。

**Step 2: 运行 HTTP/2 相关测试**

Run:
```bash
rtk cmake --build build --target t33_h2core t34_h2disp t35_h2flow t36_h2timer t30_h2err t41_h2close t85_h2pressure
rtk ctest --test-dir build -R '^http2\\.(h2core|h2disp|h2flow|h2timer|h2err|h2close|h2pressure)$' --output-on-failure
```
...
## [x] Task 13: out_sched selection 预分配优化
...
**瓶颈依据:**
- `benchmark_http2_h2_kernel_pressure 10000 1024 200000` 显示 `throughput_bottleneck_stage=scheduler`。
- 大 body 场景 `scheduler_ms=263.940`，显著高于 flow control 与 dispatcher。

**优化方案:**
- 在 `pickSendableFrames()` 进入 DATA 调度前估算本轮最多可产生的 DATA frame 数。
- 对 `H2OutboundSelection::frames` 提前 `reserve()`，队列入口还要把 control/header frame 数纳入预留。
- 不改变 DRR、公平性、END_STREAM、窗口扣减等行为。
...
Expected: 输出 scheduler QPS 与 `throughput_bottleneck_stage`，作为优化前基线。

**Step 2: 实现最小优化**

在 `out_sched.cc` 增加局部 helper：
- `estimateDataFrameCount(...)`
- `reserveSelectionFrames(...)`

要求：
- 估算只作为 reserve hint，不参与协议决策。
- 溢出时保守截断到可表示范围。
- `budget.max_frame_size == 0` 时不预留 DATA frame。

**Step 3: 验证正确性**

Run:
```bash
rtk cmake --build build --target t35_h2flow t85_h2pressure
rtk ctest --test-dir build -R '^http2\\.(h2flow|h2pressure)$' --output-on-failure
```
...
Expected: 输出优化后 scheduler QPS；将结果追加到 `docs/modules/http2/05-性能测试.md`。

## [ ] Task 14: DATA payload 拷贝路径优化方案评估

**Files:**
- Modify: `benchmark/http2/b14_h2_kernel_pressure.cc`
- Modify: `docs/modules/http2/05-性能测试.md`
- Modify later only if accepted: `src/galay-http2/protoc/http2_frame.h`
- Modify later only if accepted: `src/galay-http2/kernel/out_sched.cc`

**优化方案:**
- 评估将 `Http2DataFrame` payload 从总是 owning `std::string` 改为可选 `std::string_view`/shared buffer view 的收益和 API 风险。
- 当前 public frame API 暴露 `const std::string& data()`，直接改 view 会影响 ABI/API；必须先写迁移方案。
- 若实施，先增加不破坏现有 API 的 `Http2DataPayload` 内部表示，保持 `data()` 兼容。

**验证:**
- `t31_h2frame`、`t35_h2flow`、`t85_h2pressure` 必须通过。
- benchmark 必须证明 scheduler 大 body 场景继续改善，否则不合入。

## [x] Task 15: DATA 批量 bytes 调度路径

**Files:**
- Modify: `src/galay-http2/kernel/out_sched.h`
- Modify: `src/galay-http2/kernel/out_sched.cc`
- Modify: `benchmark/http2/b14_h2_kernel_pressure.cc`
- Add or Modify: `test/http2/t35_h2flow.cc`

**优化方案:**
- 增加可选 bytes selection：直接产出 HTTP/2 DATA frame bytes，绕过 `Http2DataFrame` 对象和 `unique_ptr` 分配。
- 保留现有 `pickSendableFrames()` 兼容路径。
- h2_core/stream_manager 后续可按写路径选择 bytes selection。
- 已实现 `Http2OutboundScheduler::pickSendableBytes()`，当前覆盖 DATA 调度路径；control/header bytes 队列可在 h2_core 写路径接入时继续补齐。

**验证:**
- bytes 输出必须与 `Http2FrameBuilder::dataBytes()` 等价。
- 大 body benchmark 中 scheduler frame QPS 必须明显提升。

## [ ] Task 16: flow_control 与 out_sched 窗口扣减合并

**Files:**
- Modify: `src/galay-http2/kernel/flow_control.h`
- Modify: `src/galay-http2/kernel/flow_control.cc`
- Modify: `src/galay-http2/kernel/out_sched.h`
- Modify: `src/galay-http2/kernel/out_sched.cc`
- Modify: `test/http2/t35_h2flow.cc`

**优化方案:**
- 增加调度器可直接接入 `H2FlowController` 的 API，一次完成 available/consume，避免上层重复维护 connection/stream window。
- 保持现有 `H2OutboundBudget` 路径用于轻量测试和兼容。
