...
typedef struct ThroughputBench {
...
static galay_status_t bench_route(const galay_http_request_t* request,
                                  galay_http_response_t* response,
                                  void* user_data)
{
    ThroughputBench* bench = (ThroughputBench*)user_data;
    if (bench == NULL || request == NULL) {
...
{
    ThroughputBench* bench = (ThroughputBench*)arg;
    for (int i = 0; i < bench->iterations; ++i) {
        C_IOResult served = galay_http_server_serve_one(bench->server, 2000);
...
static void client_entry(void* arg)
{
    ...
    for (int i = 0; i < bench->iterations; ++i) {
        ...
        if (galay_http_client_create(&client) != GALAY_OK ||
            ...
            galay_http_request_set_method_path(request, GALAY_HTTP_METHOD_GET, "/bench") != GALAY_OK ||
...
int main(int argc, char** argv)
{
    ...
    if (galay_kernel_runtime_create(&config, &runtime) != C_RuntimeSuccess ||
        ...
        galay_http_server_add_route(bench.server, GALAY_HTTP_METHOD_GET, "/bench",
                                    bench_route, &bench) != GALAY_OK) {
    ...
    if (printf("c_http_client_server iterations=%d elapsed_ns=%lld avg_us=%.2f\n",
