...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        if (galay_kernel_tcp_socket_accept_timeout(&runtime, &listener, 0, on_accept, &state) !=
            C_TcpSocketSuccess) {
            ++failures;
            break;
        }
        if (wait_done(&state.done) != 0 || atomic_load(&state.code) != (int)C_TcpSocketTimeout) {
            ++failures;
            break;
        }
        ++completed;
    ...
    printf("tcp_timeout_smoke iterations=%d completed=%d failures=%d elapsed_ns=%llu avg_ns=%.2f\n",
           iterations,
           completed,
           failures,
