...
static void accept_close_entry(void* arg)
{
    ...
    state->accept_result = galay_c_tcp_socket_accept(state->listener, &state->accepted, NULL, 1000);
...
int main(void)
{
    ...
    for (int i = 0; i < TCP_SOCKET_ITERATIONS; ++i) {
        ...
        C_IOResult spawn_result = galay_c_coro_spawn(&runtime, accept_close_entry, &state, NULL, &task);
