#include <galay/c/galay-kernel-c/async-c/tcp_socket.h>
#include <galay/c/galay-kernel-c/core-c/runtime.h>
#include <galay/c/galay-kernel-c/coro-c/coro_task.h>
#include <galay/c/galay-redis-c/redis.h>

#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>

typedef struct RedisAuthPipelineBench {
    galay_c_tcp_socket_t* listener;
    C_Host peer;
    galay_c_tcp_socket_t accepted;
    int iterations;
    int server_ok;
    int client_ok;
    int64_t elapsed_ns;
    char request[128];
} RedisAuthPipelineBench;

static int parse_iterations(int argc, char** argv)
{
    if (argc < 2) {
        return 100;
    }
    char* end = NULL;
    long parsed = strtol(argv[1], &end, 10);
    if (end == argv[1] || *end != '\0' || parsed <= 0 || parsed > 100000) {
        return -1;
    }
    return (int)parsed;
}

static int64_t monotonic_ns(void)
{
    struct timespec ts;
    if (clock_gettime(CLOCK_MONOTONIC, &ts) != 0) {
        return -1;
    }
    return (int64_t)ts.tv_sec * 1000000000LL + (int64_t)ts.tv_nsec;
}

static int create_listener(galay_c_tcp_socket_t* listener, C_Host* local)
{
    C_Host bind_host = {C_IPTypeIPV4, "127.0.0.1", 0};
    return galay_c_tcp_socket_create(listener, C_IPTypeIPV4).code == C_IOResultOk &&
        galay_c_tcp_socket_bind(listener, &bind_host).code == C_IOResultOk &&
        galay_c_tcp_socket_listen(listener, 16).code == C_IOResultOk &&
        galay_c_tcp_socket_local_endpoint(listener, local).code == C_IOResultOk &&
        local->port != 0
        ? 0
        : 1;
}

static int recv_exact(galay_c_tcp_socket_t* socket, char* buffer, size_t length)
{
    size_t received = 0;
    while (received < length) {
        C_IOResult result =
            galay_c_tcp_socket_recv(socket, buffer + received, length - received, 1000);
        if (result.code != C_IOResultOk || result.bytes == 0) {
            return 1;
        }
        received += result.bytes;
    }
    return 0;
}

static int send_exact(galay_c_tcp_socket_t* socket, const char* buffer, size_t length)
{
    size_t sent = 0;
    while (sent < length) {
        C_IOResult result =
            galay_c_tcp_socket_send(socket, buffer + sent, length - sent, 1000);
        if (result.code != C_IOResultOk || result.bytes == 0) {
            return 1;
        }
        sent += result.bytes;
    }
    return 0;
}

static int expect_request(galay_c_tcp_socket_t* socket,
                          char* buffer,
                          const char* expected,
                          size_t expected_len)
{
    return recv_exact(socket, buffer, expected_len) == 0 &&
        memcmp(buffer, expected, expected_len) == 0
        ? 0
        : 1;
}

static void server_entry(void* arg)
{
    static const char auth_request[] =
        "*3\r\n$4\r\nAUTH\r\n$7\r\ndefault\r\n$6\r\nsecret\r\n";
    static const char select_request[] = "*2\r\n$6\r\nSELECT\r\n$1\r\n2\r\n";
    static const char ping_request[] = "*1\r\n$4\r\nPING\r\n";
    static const char echo_request[] = "*2\r\n$4\r\nECHO\r\n$5\r\nhello\r\n";
    static const char ok_response[] = "+OK\r\n";
    static const char pipeline_response[] = "+PONG\r\n+hello\r\n";
    RedisAuthPipelineBench* bench = (RedisAuthPipelineBench*)arg;

    C_IOResult accepted =
        galay_c_tcp_socket_accept(bench->listener, &bench->accepted, NULL, 1000);
    if (accepted.code != C_IOResultOk) {
        return;
    }
    if (expect_request(&bench->accepted, bench->request, auth_request, sizeof(auth_request) - 1) !=
            0 ||
        send_exact(&bench->accepted, ok_response, sizeof(ok_response) - 1) != 0 ||
        expect_request(
            &bench->accepted, bench->request, select_request, sizeof(select_request) - 1) != 0 ||
        send_exact(&bench->accepted, ok_response, sizeof(ok_response) - 1) != 0) {
        return;
    }

    for (int i = 0; i < bench->iterations; ++i) {
        if (expect_request(
                &bench->accepted, bench->request, ping_request, sizeof(ping_request) - 1) != 0 ||
            expect_request(
                &bench->accepted, bench->request, echo_request, sizeof(echo_request) - 1) != 0 ||
            send_exact(&bench->accepted, pipeline_response, sizeof(pipeline_response) - 1) != 0) {
            return;
        }
    }
    C_IOResult closed = galay_c_tcp_socket_close(&bench->accepted);
    bench->server_ok = closed.code == C_IOResultOk ? 1 : 0;
}

static int validate_pipeline_replies(galay_redis_reply_t** replies, size_t reply_count)
{
    const char* first = NULL;
    const char* second = NULL;
    size_t first_len = 0;
    size_t second_len = 0;
    return reply_count == 2 &&
        replies != NULL &&
        galay_redis_reply_string(replies[0], &first, &first_len) == GALAY_OK &&
        galay_redis_reply_string(replies[1], &second, &second_len) == GALAY_OK &&
        first_len == 4 &&
        second_len == 5 &&
        memcmp(first, "PONG", first_len) == 0 &&
        memcmp(second, "hello", second_len) == 0
        ? 0
        : 1;
}

static void client_entry(void* arg)
{
    RedisAuthPipelineBench* bench = (RedisAuthPipelineBench*)arg;
    galay_redis_client_config_t config = {
        .host = bench->peer.address,
        .port = bench->peer.port,
        .username = NULL,
        .password = NULL,
        .db_index = 0,
        .resp_version = 2,
        .connect_timeout_ms = 1000,
    };
    galay_redis_client_t* client = NULL;
    galay_redis_pipeline_t* pipeline = NULL;
    const char* echo_args[] = {"hello"};

    if (galay_redis_client_create(&config, &client) != GALAY_OK || client == NULL) {
        return;
    }
    C_IOResult connected = galay_redis_client_connect(client, 1000);
    if (connected.code != C_IOResultOk) {
        galay_redis_client_destroy(client);
        return;
    }
    C_IOResult authed = galay_redis_client_auth(client, "default", "secret", 1000);
    C_IOResult selected = galay_redis_client_select(client, 2, 1000);
    if (authed.code != C_IOResultOk || selected.code != C_IOResultOk ||
        galay_redis_pipeline_create(&pipeline) != GALAY_OK ||
        galay_redis_pipeline_add_command(pipeline, "PING", NULL, NULL, 0) != GALAY_OK ||
        galay_redis_pipeline_add_command(pipeline, "ECHO", echo_args, NULL, 1) != GALAY_OK) {
        if (pipeline != NULL) {
            galay_redis_pipeline_destroy(pipeline);
        }
        C_IOResult closed = galay_redis_client_close(client, 1000);
        if (closed.code != C_IOResultOk) {
            bench->client_ok = 0;
        }
        galay_redis_client_destroy(client);
        return;
    }

    int64_t begin = monotonic_ns();
    if (begin < 0) {
        galay_redis_pipeline_destroy(pipeline);
        C_IOResult closed = galay_redis_client_close(client, 1000);
        if (closed.code != C_IOResultOk) {
            bench->client_ok = 0;
        }
        galay_redis_client_destroy(client);
        return;
    }
    for (int i = 0; i < bench->iterations; ++i) {
        galay_redis_reply_t** replies = NULL;
        size_t reply_count = 0;
        C_IOResult pipeline_result = galay_redis_client_pipeline_async(client,
                                                                      pipeline,
                                                                      1000,
                                                                      &replies,
                                                                      &reply_count);
        if (pipeline_result.code != C_IOResultOk ||
            validate_pipeline_replies(replies, reply_count) != 0) {
            if (replies != NULL) {
                galay_redis_pipeline_replies_destroy(replies, reply_count);
            }
            galay_redis_pipeline_destroy(pipeline);
            C_IOResult closed = galay_redis_client_close(client, 1000);
            if (closed.code != C_IOResultOk) {
                bench->client_ok = 0;
            }
            galay_redis_client_destroy(client);
            return;
        }
        galay_redis_pipeline_replies_destroy(replies, reply_count);
    }
    int64_t end = monotonic_ns();
    galay_redis_pipeline_destroy(pipeline);
    C_IOResult closed = galay_redis_client_close(client, 1000);
    bench->elapsed_ns = end >= begin ? end - begin : -1;
    bench->client_ok = closed.code == C_IOResultOk && bench->elapsed_ns >= 0 ? 1 : 0;
    galay_redis_client_destroy(client);
}

int main(int argc, char** argv)
{
    int iterations = parse_iterations(argc, argv);
    if (iterations <= 0) {
        if (fprintf(stderr, "usage: %s [iterations]\n", argv[0]) < 0) {
            return 2;
        }
        return 1;
    }

    C_RuntimeConfig config = galay_c_runtime_config_default();
    config.io_scheduler_count = 1;
    config.parallel_scheduler_count = 0;

    galay_c_runtime_t runtime = {0};
    galay_c_tcp_socket_t listener = {0};
    C_Host local = {0};
    RedisAuthPipelineBench bench = {0};
    galay_c_coro_task_t server = {0};
    galay_c_coro_task_t client = {0};
    int exit_code = 0;

    if (galay_c_runtime_create(&config, &runtime) != C_RuntimeSuccess ||
        galay_c_runtime_start(&runtime) != C_RuntimeSuccess ||
        create_listener(&listener, &local) != 0) {
        exit_code = 3;
        goto cleanup;
    }
    bench.listener = &listener;
    bench.peer = local;
    bench.iterations = iterations;

    if (galay_c_coro_spawn(&runtime, server_entry, &bench, NULL, &server).code != C_IOResultOk ||
        galay_c_coro_spawn(&runtime, client_entry, &bench, NULL, &client).code != C_IOResultOk ||
        galay_c_coro_join(&server, 5000).code != C_IOResultOk ||
        galay_c_coro_join(&client, 5000).code != C_IOResultOk ||
        bench.server_ok != 1 ||
        bench.client_ok != 1) {
        exit_code = 4;
        goto cleanup;
    }

    double avg_us = (double)bench.elapsed_ns / (double)iterations / 1000.0;
    if (printf("c_redis_auth_pipeline iterations=%d commands=%d elapsed_ns=%lld avg_us=%.2f\n",
               iterations,
               iterations * 2,
               (long long)bench.elapsed_ns,
               avg_us) < 0) {
        exit_code = 5;
    }

cleanup:
    if (server.task != NULL && galay_c_coro_destroy(&server).code != C_IOResultOk &&
        exit_code == 0) {
        exit_code = 6;
    }
    if (client.task != NULL && galay_c_coro_destroy(&client).code != C_IOResultOk &&
        exit_code == 0) {
        exit_code = 7;
    }
    if (bench.accepted.fd >= 0 &&
        galay_c_tcp_socket_close(&bench.accepted).code != C_IOResultOk &&
        exit_code == 0) {
        exit_code = 8;
    }
    if (listener.fd >= 0 &&
        galay_c_tcp_socket_close(&listener).code != C_IOResultOk &&
        exit_code == 0) {
        exit_code = 9;
    }
    if (runtime.runtime != NULL) {
        if (galay_c_runtime_stop(&runtime) != C_RuntimeSuccess && exit_code == 0) {
            exit_code = 10;
        }
        if (galay_c_runtime_destroy(&runtime) != C_RuntimeSuccess && exit_code == 0) {
            exit_code = 11;
        }
    }
    return exit_code;
}
