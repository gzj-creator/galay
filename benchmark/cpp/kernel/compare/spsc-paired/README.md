...
本次性能结论只使用 `raw_bounded` 与 `batch_bounded`。`unbounded` 保留为显式指定时可运行
的兼容 case，不在 runner 默认集合中，也不进入“SPSC 有界队列”结论。
...
| `batch_bounded` | `Ring::trySendBatch` / `tryRecvBatch` | `write_chunk_uninit` / `read_chunk` | `equivalent_measured_api` |
...
`batch_bounded` 使用同一 ring、容量和最大 batch 大小；最后不足一个 batch 的消息仍须全部
发送和接收。两侧只把返回 `0`/无可用槽位的调用计为 retry，部分批次会立即继续处理，
并保持与逐条 case 相同的 FIFO、接收数和 64 位回绕 checksum 校验。
...
- consumer 必须按预期消息数完成最终 drain；FIFO、接收数和 64 位回绕 checksum 必须
  完全匹配，不能用 producer 完成后的单次空读提前结束。
- C++ 和 Rust 进程严格串行运行。runner 先校准同一消息数，使两侧正式样本均不少于
  1 秒，再预热并默认执行 15 个 ABBA 配对样本。
...
- schema `galay.spsc.paired.v3` 的 `implementation`、`api_profile`、
  `comparison_scope`、`batch_size` 和 `backoff` 会由 runner 按 case 精确校验；旧 v2
  结果直接拒绝，防止混用协议或悄悄扩大结论。

默认稳定胜出门槛同时要求：配对吞吐比的 95% bootstrap CI 下界大于 `1.05`，且 C++、
Rust 两侧样本 CV 都不超过 `3%`、线程放置全部为 `pinned`、两侧 `empty_retries` 中位数
比值不超过 `10`。任一条件失败时，只能报告观测中位数和噪声，不能宣称稳定胜出。
跨容量结论还必须让 `64`、`256`、`4096`、`65536` 四组独立运行全部通过，不能挑选
单个容量概括成“全面超过”。
...
summary JSON。正式 Linux 对照需要对三个 backoff、四个容量分别使用独立输出目录运行，
不得让后一次结果覆盖前一次结果。
