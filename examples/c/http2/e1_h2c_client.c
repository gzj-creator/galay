...
static int make_headers(galay_http2_headers_t** out, const char* path)
{
...
{
    static const char response[] = "h2c-client-example";
    ...
    if (state->server_result.code == C_IOResultOk &&
        galay_http2_stream_read_headers(stream, &headers, 0).code == C_IOResultOk &&
...
static void client_entry(void* arg)
{
    ...
    state->client_result =
        galay_http2_client_open_stream(client, headers, GALAY_TRUE, &stream, 2000);
    if (state->client_result.code == C_IOResultOk &&
...
int main(void)
{
    ...
    if (galay_kernel_runtime_create(&runtime_config, &runtime) != C_RuntimeSuccess ||
        ...
        state.client_result.code != C_IOResultOk) {
        exit_code = 1;
    } else if (printf("h2c_client response=%.*s\n", (int)state.body_len, state.body) < 0) {
        exit_code = 2;
    }
