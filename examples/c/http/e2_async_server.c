...
int main(void)
{
    ...
    if (galay_c_coro_spawn(&runtime, server_entry, &example, NULL, &server_task).code != C_IOResultOk ||
        galay_c_coro_spawn(&runtime, client_probe_entry, &example, NULL, &client_task).code != C_IOResultOk ||
