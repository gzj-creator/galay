...
int main(void)
{
    ...
    if (galay_etcd_config_builder_set_endpoint(builder, "http://127.0.0.1:2379") != GALAY_OK ||
        galay_etcd_config_builder_set_request_timeout_ms(builder, 1000) != GALAY_OK ||
        galay_etcd_client_create(builder, &client) != GALAY_OK) {
        galay_etcd_config_builder_destroy(builder);
        return 1;
    }

    printf("c etcd client configured: connected=%d\n", (int)galay_etcd_client_connected(client));
