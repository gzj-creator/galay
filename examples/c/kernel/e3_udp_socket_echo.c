...
static void send_entry(void* ctx)
{
    ...
    state->result =
        galay_c_udp_socket_sendto(state->socket,
                                       state->buffer,
                                       state->length,
                                       &state->to,
                                       2000);
...
static void close_entry(void* ctx)
{
    ...
    state->result = galay_c_udp_socket_close(state->socket);
}

static int close_socket(galay_c_runtime_t* runtime, galay_c_udp_socket_t* socket)
{
    if (socket->fd < 0) {
        return 0;
    }
    ...
    if (galay_c_coro_spawn(runtime, close_entry, &state, 0, &task).code != C_IOResultOk ||
...
int main(void)
{
    ...
    galay_c_udp_socket_t server = {.fd = -1};
    galay_c_udp_socket_t client = {.fd = -1};
    galay_c_coro_task_t recv_task = {0};
    galay_c_coro_task_t send_task = {0};
    ...
    if (galay_c_runtime_create(&config, &runtime) != C_RuntimeSuccess ||
        ...
        galay_c_udp_socket_create(&server, C_IPTypeIPV4).code != C_IOResultOk ||
        galay_c_udp_socket_create(&client, C_IPTypeIPV4).code != C_IOResultOk ||
        galay_c_udp_socket_bind(&server, &bind_host).code != C_IOResultOk ||
        galay_c_udp_socket_bind(&client, &bind_host).code != C_IOResultOk ||
        galay_c_udp_socket_local_endpoint(&server, &server_local).code != C_IOResultOk ||
        galay_c_udp_socket_local_endpoint(&client, &client_local).code != C_IOResultOk) {
        exit_code = 1;
        goto cleanup;
    }
    ...
    if (galay_c_coro_spawn(&runtime, recv_entry, &server_recv, 0, &recv_task).code != C_IOResultOk ||
        galay_c_coro_spawn(&runtime, send_entry, &client_send, 0, &send_task).code != C_IOResultOk ||
        galay_c_coro_join(&send_task, 3000).code != C_IOResultOk ||
        galay_c_coro_join(&recv_task, 3000).code != C_IOResultOk ||
    ...
    if (galay_c_coro_destroy(&send_task).code != C_IOResultOk ||
        galay_c_coro_destroy(&recv_task).code != C_IOResultOk) {
        exit_code = 4;
        goto cleanup;
    }
    ...
    if (galay_c_coro_spawn(&runtime, recv_entry, &client_recv, 0, &recv_task).code != C_IOResultOk ||
        galay_c_coro_spawn(&runtime, send_entry, &server_send, 0, &send_task).code != C_IOResultOk ||
        galay_c_coro_join(&send_task, 3000).code != C_IOResultOk ||
        galay_c_coro_join(&recv_task, 3000).code != C_IOResultOk ||
...
cleanup:
    if (send_task.task != 0) {
        if (galay_c_coro_destroy(&send_task).code != C_IOResultOk && exit_code == 0) {
            exit_code = 6;
        }
    }
    if (recv_task.task != 0) {
        if (galay_c_coro_destroy(&recv_task).code != C_IOResultOk && exit_code == 0) {
            exit_code = 7;
        }
    }
    ...
    if (client.fd >= 0) {
        if (galay_c_udp_socket_close(&client).code != C_IOResultOk && exit_code == 0) {
            exit_code = 10;
        }
    }
    if (server.fd >= 0) {
        if (galay_c_udp_socket_close(&server).code != C_IOResultOk && exit_code == 0) {
            exit_code = 11;
        }
    }
    if (runtime.runtime != 0) {
