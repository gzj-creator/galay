...
static void server_entry(void* ctx)
{
    ...
    state->accept_result =
        galay_c_tcp_socket_accept(state->listener, &state->accepted, 0, 2000);
    if (state->accept_result.code != C_IOResultOk) {
        return;
    }
    state->recv_result =
        galay_c_tcp_socket_recv(&state->accepted, state->recv_buffer, sizeof(state->recv_buffer), 2000);
    if (state->recv_result.code != C_IOResultOk) {
        return;
    }
    state->send_result =
        galay_c_tcp_socket_send(&state->accepted, response, sizeof(response) - 1, 2000);
...
cleanup:
    ...
    if (state.accepted.fd >= 0) {
        if (galay_c_tcp_socket_close(&state.accepted).code != C_IOResultOk && exit_code == 0) {
            exit_code = 6;
        }
    }
    if (listener.fd >= 0) {
