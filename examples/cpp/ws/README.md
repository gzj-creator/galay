...
- `mcpp/`：模块门禁满足时启用的 import 示例实现（TLS 场景还需要 `-DGALAY_BUILD_SSL=ON`）
...
```bash
cmake -S . -B build_ssl -DBUILD_EXAMPLES=ON -DGALAY_BUILD_SSL=ON
...
- `mcpp/` 在模块门禁满足时覆盖与 `include/` 树对齐的 E1~E14 场景
- 其中 `E5`~`E8`、`E13`、`E14` 还需要 `-DGALAY_BUILD_SSL=ON`
