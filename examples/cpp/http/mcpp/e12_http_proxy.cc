...
int main(int argc, char* argv[]) {
    uint16_t listen_port = galay::http::example::kDefaultProxyPort;
    std::string upstream_host = "127.0.0.1";
    uint16_t upstream_port = galay::http::example::kDefaultProxyUpstreamPort;
    ...
    const bool mount_enabled = !(mount_prefix == "off" || mount_prefix == "none" ||
    ...
    HttpServer server(HttpServerBuilder()
        ...
        .ioSchedulerCount(2)
        .build());
    std::cout << "Import proxy listen: http://127.0.0.1:" << listen_port << "\n";
